...
router.all('/ass', function (req, res, next) {
    ...
    if (selectParams.date2 == undefined){
        selectParams.date2 ="2122-01-01";
    }
    let whereSql = "  and ddh like '%" + selectParams.ddh + "%' and ifnull(syy,'') like '%" + selectParams.syy + "%' and ifnull(hyxm,'') like '%" + selectParams.hyxm + "%' and riqi>='" + selectParams.date1 + "' and riqi<='" + selectParams.date2 + "' and hyzh ='" + username + "'"

    let sql1 = "select Count(*) as count from orders where company = '" + company + "'" + whereSql;
...
router.post('/update', function (req, res) {
    ...
    if (req.body.checkForm) {
        ...
        let sql = "update orders set riqi='" + riqi + "',ddh='" + ddh + "',hyzh='" + hyzh + "',hyxm='" + hyxm + "',yhfa='" + yhfa + "',xfje='" + xfje + "',ssje='" + ssje + "',yhje='" + yhje + "',syy='" + syy + "',company='" + company + "' where id=" + id;
        console.log("sql:"+sql)
